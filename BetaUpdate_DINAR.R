...
BetaUpdate_DINAR = function(iset, b, lambda11, lambda22, Yx, Yr, alpha) {
  ...
  for (i in iset) {
    ...
    # set all parameters on the diagonal or in a column to 0 if DINAR penalty dictates so
    if (i == 0 & sqrt(sum((sign(testval) * pmax(abs(testval) - lambda11*alpha,0))^2)) <= lambda11*(1-alpha)) {
      ...
      if (i == 0) {
        ...
        while (any(abs(beta - bupdate) > 1e-6)) { 
          # for details on the parameter update, see appendix in DINAR paper
          beta = bupdate
          tval = 1
