...
#####################################################################################
#
# 1. part of code which identifies the active columns. For details, see the 3 step algorithm in the DINAR paper
############################################
max.iterations = 100
eps = 1e-6
...
for (j in 1:length(lambda1)) {
  ...
  for (co in 1:length(lambda2)) {
    ...
    while (any(abs(b - bfull) > eps) & k<=max.iterations) {
      ...
      print(paste(co, "|", k))
      bfull = b 
      b = BetaUpdate_DINAR(0:N, b_old, lambda1[j], lambda2[co], Yx, Yr, alpha) 
      b_old = b
    }
    erg[[co]] = list(b)
    iterate[j,co]=k
...
# 2. part of code which computes the parameter matrix unpenalized. The result serves as starting value for the DINAR estimation
...
for(j in 1:length(lambda1)) {
  ...
  for (co in 1:dim(unique_which_columns)[1]) {
    ...
    while (any(abs(b - bfull) > eps2) & k<=max.iterations) {
      ...
      print(paste(co, "|", k))
      bfull = b 
      b = BetaUpdate_DINAR(iset, b_old, 0, 0, Yx, Yr, alpha) 
      b_old = b
    }
    erg[[co]] = list(b)
    iterate[j,co]=k
...
# 3. part of code which derives the parameter matrix with DINAR, using parts 1. and 2. to improve runtime
...
for(j in 1:length(lambda1)) {
  ...
  for (co in 1:length(lambda2)) {
    ...
    while (any(abs(b - bfull) > eps2) & k<=max.iterations) {
      ...
      print(paste(co, "|", k))
      bfull = b 
      b = BetaUpdate_DINAR(iset, b_old, lambda1[j], lambda2[co], Yx, Yr, alpha) 
      b_old = b
    }
    erg[[co]] = list(b)
    iterate[j,co]=k
